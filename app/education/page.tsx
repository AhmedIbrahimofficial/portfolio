...
const card  = { background: "rgba(10,10,20,0.55)", backdropFilter: "blur(12px)", borderColor: "rgba(255,255,255,0.08)" } as React.CSSProperties;
const card2 = { background: "rgba(10,10,20,0.45)", backdropFilter: "blur(8px)",  borderColor: "rgba(255,255,255,0.07)" } as React.CSSProperties;

export default function EducationPage() {
  return (
    <div style={{ background: "#04040a", minHeight: "100vh", position: "relative" }}>
      <BrainBackground />
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <section className="pt-32 pb-16 px-6">
          <div className="max-w-5xl mx-auto">
            ...
            <p className="text-base md:text-lg max-w-2xl leading-relaxed font-medium" style={{ color: "rgba(255,255,255,0.6)" }}>
        ...
        <section className="py-16 px-6">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="rounded-2xl border p-8 relative overflow-hidden" style={card}>
              ...
              <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-2">
                  ...
                  <p className="text-sm mb-4 font-medium" style={{ color: "rgba(255,255,255,0.6)" }}>
                ...
                <div className="space-y-4">
                  {[
                    { label: "Duration",    value: "May 2024 – May 2025" },
                    { label: "Grade",       value: "Grade 8" },
                    { label: "Institution", value: "APSACS" },
                    { label: "Field",       value: "Artificial Intelligence" },
                  ].map((item) => (
                    <div key={item.label}>
                      <span className="text-[10px] uppercase tracking-[0.2em] block mb-1 font-bold" style={{ color: "rgba(255,255,255,0.35)" }}>{item.label}</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {subjects.map((subject) => (
                <div key={subject.name}
                  className="p-6 rounded-2xl border transition-all duration-300 hover:border-[#89AACC]/50 hover:-translate-y-1"
                  style={card2}>
                  ...
                  <p className="text-xs leading-relaxed font-medium" style={{ color: "rgba(255,255,255,0.55)" }}>{subject.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="space-y-3">
              {selfLearning.map((item, idx) => (
                <div key={item.topic}
                  className="flex items-center justify-between p-5 rounded-xl border transition-all duration-200 hover:border-[#89AACC]/50"
                  style={card2}>
                  <div className="flex items-center gap-4">
                    <span className="text-xs font-mono w-6 text-right font-bold" style={{ color: "rgba(255,255,255,0.25)" }}>
                      {String(idx + 1).padStart(2, "0")}
                    </span>
                    <div>
                      <p className="text-sm font-bold" style={{ color: "#ffffff" }}>{item.topic}</p>
                      <p className="text-xs mt-0.5 font-medium" style={{ color: "rgba(255,255,255,0.45)" }}>{item.source}</p>
                    </div>
                  </div>
                  <span className="text-xs rounded-full px-3 py-1 border flex-shrink-0 font-semibold"
                    style={{ background: "rgba(137,170,204,0.1)", borderColor: "rgba(137,170,204,0.25)", color: "#89AACC" }}>
                    {item.year}
                  </span>
                </div>
              ))}
        ...
        <section className="py-16 px-6">
          <div className="max-w-5xl mx-auto">
            <div className="rounded-2xl border p-8 md:p-12 text-center relative overflow-hidden" style={card}>
              ...
              <div className="relative z-10">
                ...
                <p className="text-sm mt-6 font-semibold" style={{ color: "rgba(255,255,255,0.45)" }}>— Ahmed Ibrahim</p>
              </div>
            </div>
          </div>
        </section>
